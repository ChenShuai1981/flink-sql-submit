 ...
 * will use {@link RetractableFlinkKafkaProducer.Semantic#AT_LEAST_ONCE} semantic.
 * Before using {@link RetractableFlinkKafkaProducer.Semantic#EXACTLY_ONCE} please refer to Flink's
 * Kafka connector documentation.
 */
@PublicEvolving
public class RetractableFlinkKafkaProducer<IN>
        extends TwoPhaseCommitSinkFunction<Tuple2<Boolean, IN>, RetractableFlinkKafkaProducer.KafkaTransactionState, RetractableFlinkKafkaProducer.KafkaTransactionContext> {
    ...
    public enum Semantic {
         ...
         *
         * <p>In this mode {@link RetractableFlinkKafkaProducer} sets up a pool of {@link FlinkKafkaInternalProducer}. Between each
         * checkpoint a Kafka transaction is created, which is committed on
         * {@link RetractableFlinkKafkaProducer#notifyCheckpointComplete(long)}. If checkpoint complete notifications are
         * running late, {@link RetractableFlinkKafkaProducer} can run out of {@link FlinkKafkaInternalProducer}s in the pool. In that
         * case any subsequent {@link RetractableFlinkKafkaProducer#snapshotState(FunctionSnapshotContext)} requests will fail
         * and {@link RetractableFlinkKafkaProducer} will keep using the {@link FlinkKafkaInternalProducer}
    ...
    private static final Logger LOG = LoggerFactory.getLogger(RetractableFlinkKafkaProducer.class);
     ...
     * of {@link RetractableFlinkKafkaProducer} from scratch without clean shutdown of the previous one,
     * {@link RetractableFlinkKafkaProducer} doesn't know what was the set of previously used Kafka's transactionalId's. In
    ...
    /**
     * Default number of KafkaProducers in the pool. See {@link RetractableFlinkKafkaProducer.Semantic#EXACTLY_ONCE}.
     */
     ...
     */
    @Deprecated
    private static final ListStateDescriptor<RetractableFlinkKafkaProducer.NextTransactionalIdHint> NEXT_TRANSACTIONAL_ID_HINT_DESCRIPTOR =
            new ListStateDescriptor<>("next-transactional-id-hint", TypeInformation.of(NextTransactionalIdHint.class));

    private static final ListStateDescriptor<RetractableFlinkKafkaProducer.NextTransactionalIdHint> NEXT_TRANSACTIONAL_ID_HINT_DESCRIPTOR_V2 =
     ...
     */
    private transient ListState<RetractableFlinkKafkaProducer.NextTransactionalIdHint> nextTransactionalIdHintState;
     ...
     */
    private transient RetractableFlinkKafkaProducer.NextTransactionalIdHint nextTransactionalIdHint;
     ...
     */
    protected RetractableFlinkKafkaProducer.Semantic semantic;
    ...
    /**
     * Creates a RetractableFlinkKafkaProducer for a given topic. The sink produces a DataStream to
     * the topic.
     *
     * @param brokerList
     ...
     *
     * @deprecated use {@link #RetractableFlinkKafkaProducer(String, KafkaSerializationSchema, Properties, RetractableFlinkKafkaProducer.Semantic)}
     */
    @Deprecated
    public RetractableFlinkKafkaProducer(String brokerList, String topicId, SerializationSchema<IN> serializationSchema) {
        this(
                topicId,
     ...
     * {@link #RetractableFlinkKafkaProducer(String, SerializationSchema, Properties, Optional)} instead.
     *
     * @param topicId
     * 			ID of the Kafka topic.
     * @param serializationSchema
     ...
     */
    @Deprecated
    public RetractableFlinkKafkaProducer(String topicId, SerializationSchema<IN> serializationSchema, Properties producerConfig) {
        this(
                topicId,
                new KeyedSerializationSchemaWrapper<>(serializationSchema),
                producerConfig,
    ...
    /**
     * Creates a RetractableFlinkKafkaProducer for a given topic. The sink produces its input to
     ...
     */
    @Deprecated
    public RetractableFlinkKafkaProducer(
            String topicId,
     ...
     *
     * <p>To use a custom partitioner, please use
     * {@link #RetractableFlinkKafkaProducer(String, KeyedSerializationSchema, Properties, Optional)} instead.
     *
     * @param brokerList
     *			Comma separated addresses of the brokers
     * @param topicId
     ...
     */
    @Deprecated
    public RetractableFlinkKafkaProducer(String brokerList, String topicId, KeyedSerializationSchema<IN> serializationSchema) {
        this(
                topicId,
                serializationSchema,
                getPropertiesFromBrokerList(brokerList),
     ...
     */
    @Deprecated
    public RetractableFlinkKafkaProducer(String topicId, KeyedSerializationSchema<IN> serializationSchema, Properties producerConfig) {
        this(
                topicId,
                serializationSchema,
                producerConfig,
     ...
     * 			Properties with the producer configuration.
     * @param semantic
     * 			Defines semantic that will be used by this producer (see {@link RetractableFlinkKafkaProducer.Semantic}).
     *
     * @deprecated use {@link #RetractableFlinkKafkaProducer(String, KafkaSerializationSchema, Properties, RetractableFlinkKafkaProducer.Semantic)}
     */
    @Deprecated
    public RetractableFlinkKafkaProducer(
            ...
            Properties producerConfig,
            RetractableFlinkKafkaProducer.Semantic semantic) {
        this(topicId,
                serializationSchema,
                producerConfig,
                Optional.of(new FlinkFixedPartitioner<IN>()),
    ...
    public RetractableFlinkKafkaProducer(
        ...
        this(
                ...
                producerConfig,
                customPartitioner,
                RetractableFlinkKafkaProducer.Semantic.AT_LEAST_ONCE,
                DEFAULT_KAFKA_PRODUCERS_POOL_SIZE);
    }
     ...
     *                          are {@code null}, then records will be distributed to Kafka partitions in a
     *                          round-robin fashion.
     * @param semantic Defines semantic that will be used by this producer (see {@link RetractableFlinkKafkaProducer.Semantic}).
     * @param kafkaProducersPoolSize Overwrite default KafkaProducers pool size (see {@link RetractableFlinkKafkaProducer.Semantic#EXACTLY_ONCE}).
     *
     * @deprecated use {@link #RetractableFlinkKafkaProducer(String, KafkaSerializationSchema, Properties, RetractableFlinkKafkaProducer.Semantic)}
     */
    @Deprecated
    ...
    /**
     * Creates a {@link RetractableFlinkKafkaProducer} for a given topic. The sink produces its input to
     ...
     * @param kafkaProducersPoolSize Overwrite default KafkaProducers pool size (see {@link RetractableFlinkKafkaProducer.Semantic#EXACTLY_ONCE}).
     */
    private RetractableFlinkKafkaProducer(
            String defaultTopic,
            ...
            RetractableFlinkKafkaProducer.Semantic semantic,
            int kafkaProducersPoolSize) {
        super(new RetractableFlinkKafkaProducer.TransactionStateSerializer(), new RetractableFlinkKafkaProducer.ContextStateSerializer());
        ...
        if (semantic == RetractableFlinkKafkaProducer.Semantic.EXACTLY_ONCE) {
     ...
     */
    @Override
    public RetractableFlinkKafkaProducer<IN> ignoreFailuresAfterTransactionTimeout() {
    ...
    @Override
    public void invoke(RetractableFlinkKafkaProducer.KafkaTransactionState transaction, Tuple2<Boolean, IN> value, Context context) throws FlinkKafkaException {
        checkErroneous();

        ProducerRecord<byte[], byte[]> record;
        Boolean isAdd = value.f0;
        IN next = value.f1;
        if (keyedSchema != null) {
            ...
            if (!isAdd) {
                byte[] serializedValue = null;
                record = new ProducerRecord(record.topic(), record.partition(), record.timestamp(), record.key(), serializedValue);
            }
        } else {
    ...
    @Override
    protected RetractableFlinkKafkaProducer.KafkaTransactionState beginTransaction() throws FlinkKafkaException {
        switch (semantic) {
            case EXACTLY_ONCE:
                FlinkKafkaInternalProducer<byte[], byte[]> producer = createTransactionalProducer();
                producer.beginTransaction();
                return new RetractableFlinkKafkaProducer.KafkaTransactionState(producer.getTransactionalId(), producer);
            case AT_LEAST_ONCE:
            case NONE:
                // Do not create new producer on each beginTransaction() if it is not necessary
                final RetractableFlinkKafkaProducer.KafkaTransactionState currentTransaction = currentTransaction();
                if (currentTransaction != null && currentTransaction.producer != null) {
                    return new RetractableFlinkKafkaProducer.KafkaTransactionState(currentTransaction.producer);
                }
                return new RetractableFlinkKafkaProducer.KafkaTransactionState(initNonTransactionalProducer(true));
            default:
    ...
    @Override
    protected void preCommit(RetractableFlinkKafkaProducer.KafkaTransactionState transaction) throws FlinkKafkaException {
        switch (semantic) {
            case EXACTLY_ONCE:
            case AT_LEAST_ONCE:
                flush(transaction);
    ...
    @Override
    protected void commit(RetractableFlinkKafkaProducer.KafkaTransactionState transaction) {
    ...
    @Override
    protected void recoverAndCommit(RetractableFlinkKafkaProducer.KafkaTransactionState transaction) {
        if (transaction.isTransactional()) {
            try (
    ...
    @Override
    protected void abort(RetractableFlinkKafkaProducer.KafkaTransactionState transaction) {
        if (transaction.isTransactional()) {
    ...
    @Override
    protected void recoverAndAbort(RetractableFlinkKafkaProducer.KafkaTransactionState transaction) {
        if (transaction.isTransactional()) {
            try (
                    FlinkKafkaInternalProducer<byte[], byte[]> producer =
                            initTransactionalProducer(transaction.transactionalId, false)) {
     ...
     * @param transaction
     */
    private void flush(RetractableFlinkKafkaProducer.KafkaTransactionState transaction) throws FlinkKafkaException {
    ...
    public void snapshotState(FunctionSnapshotContext context) throws Exception {
        ...
        if (getRuntimeContext().getIndexOfThisSubtask() == 0 && semantic == RetractableFlinkKafkaProducer.Semantic.EXACTLY_ONCE) {
            ...
            nextTransactionalIdHintState.add(new RetractableFlinkKafkaProducer.NextTransactionalIdHint(
    ...
    public void initializeState(FunctionInitializationContext context) throws Exception {
        if (semantic != RetractableFlinkKafkaProducer.Semantic.NONE && !((StreamingRuntimeContext) this.getRuntimeContext()).isCheckpointingEnabled()) {
            LOG.warn("Using {} semantic, but checkpointing is not enabled. Switching to {} semantic.", semantic, RetractableFlinkKafkaProducer.Semantic.NONE);
            semantic = RetractableFlinkKafkaProducer.Semantic.NONE;
        }
        ...
        if (semantic != RetractableFlinkKafkaProducer.Semantic.EXACTLY_ONCE) {
            ...
            ArrayList<RetractableFlinkKafkaProducer.NextTransactionalIdHint> transactionalIdHints = Lists.newArrayList(nextTransactionalIdHintState.get());
            if (transactionalIdHints.size() > 1) {
                ...
                nextTransactionalIdHint = new RetractableFlinkKafkaProducer.NextTransactionalIdHint(0, 0);
    ...
    @Override
    protected Optional<RetractableFlinkKafkaProducer.KafkaTransactionContext> initializeUserContext() {
        if (semantic != RetractableFlinkKafkaProducer.Semantic.EXACTLY_ONCE) {
        ...
        return Optional.of(new RetractableFlinkKafkaProducer.KafkaTransactionContext(transactionalIds));
    }
    ...
    static class KafkaTransactionState {
        ...
        public boolean equals(Object o) {
            ...
            RetractableFlinkKafkaProducer.KafkaTransactionState that = (RetractableFlinkKafkaProducer.KafkaTransactionState) o;
    ...
    /**
     * Context associated to this instance of the {@link RetractableFlinkKafkaProducer}. User for keeping track of the
     * transactionalIds.
     */
    @VisibleForTesting
    @Internal
    public static class KafkaTransactionContext {
        ...
        public boolean equals(Object o) {
            ...
            RetractableFlinkKafkaProducer.KafkaTransactionContext that = (RetractableFlinkKafkaProducer.KafkaTransactionContext) o;
     ...
     * {@link RetractableFlinkKafkaProducer.KafkaTransactionState}.
     */
    @VisibleForTesting
    @Internal
    public static class TransactionStateSerializer extends TypeSerializerSingleton<RetractableFlinkKafkaProducer.KafkaTransactionState> {
        ...
        @Override
        public RetractableFlinkKafkaProducer.KafkaTransactionState createInstance() {
            return null;
        }

        @Override
        public RetractableFlinkKafkaProducer.KafkaTransactionState copy(RetractableFlinkKafkaProducer.KafkaTransactionState from) {
            return from;
        }

        @Override
        public RetractableFlinkKafkaProducer.KafkaTransactionState copy(
                RetractableFlinkKafkaProducer.KafkaTransactionState from,
                RetractableFlinkKafkaProducer.KafkaTransactionState reuse) {
            return from;
        }
        ...
        public void serialize(
                RetractableFlinkKafkaProducer.KafkaTransactionState record,
        ...
        @Override
        public RetractableFlinkKafkaProducer.KafkaTransactionState deserialize(DataInputView source) throws IOException {
            ...
            return new RetractableFlinkKafkaProducer.KafkaTransactionState(transactionalId, producerId, epoch, null);
        }

        @Override
        public RetractableFlinkKafkaProducer.KafkaTransactionState deserialize(
                RetractableFlinkKafkaProducer.KafkaTransactionState reuse,
        ...
        @Override
        public TypeSerializerSnapshot<RetractableFlinkKafkaProducer.KafkaTransactionState> snapshotConfiguration() {
        ...
        public static final class TransactionStateSerializerSnapshot extends SimpleTypeSerializerSnapshot<RetractableFlinkKafkaProducer.KafkaTransactionState> {
    ...
    /**
     * {@link org.apache.flink.api.common.typeutils.TypeSerializer} for
     * {@link RetractableFlinkKafkaProducer.KafkaTransactionContext}.
     */
    @VisibleForTesting
    @Internal
    public static class ContextStateSerializer extends TypeSerializerSingleton<RetractableFlinkKafkaProducer.KafkaTransactionContext> {
        ...
        @Override
        public RetractableFlinkKafkaProducer.KafkaTransactionContext createInstance() {
            return null;
        }

        @Override
        public RetractableFlinkKafkaProducer.KafkaTransactionContext copy(RetractableFlinkKafkaProducer.KafkaTransactionContext from) {
            return from;
        }

        @Override
        public RetractableFlinkKafkaProducer.KafkaTransactionContext copy(
                RetractableFlinkKafkaProducer.KafkaTransactionContext from,
                RetractableFlinkKafkaProducer.KafkaTransactionContext reuse) {
            return from;
        }
        ...
        @Override
        public void serialize(
                RetractableFlinkKafkaProducer.KafkaTransactionContext record,
                DataOutputView target) throws IOException {
        ...
        @Override
        public RetractableFlinkKafkaProducer.KafkaTransactionContext deserialize(DataInputView source) throws IOException {
            int numIds = source.readInt();
            Set<String> ids = new HashSet<>(numIds);
            for (int i = 0; i < numIds; i++) {
                ids.add(source.readUTF());
            }
            return new RetractableFlinkKafkaProducer.KafkaTransactionContext(ids);
        }

        @Override
        public RetractableFlinkKafkaProducer.KafkaTransactionContext deserialize(
                RetractableFlinkKafkaProducer.KafkaTransactionContext reuse,
                DataInputView source) throws IOException {
            return deserialize(source);
        }
    ...
    /**
     * {@link org.apache.flink.api.common.typeutils.TypeSerializer} for
     * {@link RetractableFlinkKafkaProducer.NextTransactionalIdHint}.
     */
    @VisibleForTesting
    @Internal
